...
interface OPDRegistrationSupabase {
  ...
  patient_detail: PatientDetailFromSupabase | null // Joined data is an object, not array
  // Add these if they are actually columns in your opd_registration table and you select them
...
interface IPDRegistrationSupabase {
  ...
  created_at: string // timestamp with time zone (UTC ISO string)
  bed_id: number | null;
  bed_management: { room_type: string } | null; // Joined data
  patient_detail: PatientDetailFromSupabase | null // Joined data is an object
}
...
const getTodayDateRange = () => {
  const now = new Date(); // Current system time (UTC)
  const todayInIST = toIST(now); // Get current date/time converted to IST
  return getDayRangeUtcFromLocal(format(todayInIST, "yyyy-MM-dd"));
};
...
const DashboardPage: React.FC = () => {
  ...
  const currentDateRange = useMemo(() => {
    let rangeUtc: { start: string; end: string; };
    let displayRangeStart: string;
    let displayRangeEnd: string;

    switch (filters.filterType) {
      case "today":
        rangeUtc = getTodayDateRange();
        const todayIST = toIST(parseISO(rangeUtc.start)); // Convert UTC start to IST for display
        displayRangeStart = format(todayIST, "yyyy-MM-dd");
        displayRangeEnd = format(todayIST, "yyyy-MM-dd");
        break;
      case "month":
        ...
        displayRangeStart = format(monthStartIST, "yyyy-MM-dd");
        displayRangeEnd = format(monthEndIST, "yyyy-MM-dd");
        break;
      case "dateRange":
        ...
        displayRangeStart = filters.startDate;
        displayRangeEnd = filters.endDate;
        break;
      case "week":
      default:
        ...
        displayRangeStart = format(weekStartIST, "yyyy-MM-dd");
        displayRangeEnd = format(weekEndIST, "yyyy-MM-dd");
        break;
    }
    ...
    return {
      ...
      displayStart: displayRangeStart, // For display in filters
      displayEnd: displayRangeEnd,     // For display in filters
    };
  }, [filters]);
  ...
  useEffect(() => {
    const fetchDoctors = async () => {
      try {
        const { data, error } = await supabase.from("doctor").select("id, dr_name, department, specialist, charges")
        if (error) throw error
        const doctorsMap: { [key: string]: Doctor } = {}
        data.forEach((doc) => {
          doctorsMap[String(doc.id)] = { // Ensure doctor ID is string for map key
            id: String(doc.id),
            dr_name: doc.dr_name,
            department: doc.department,
            specialist: doc.specialist,
            opd_charge: doc.charges?.opd_charge || 0,
          }
        })
  ...
  useEffect(() => {
      ...
      if (filters.searchQuery) {
        ...
        try {
          const { data, error } = await supabase
            ...
            .or(
              `name.ilike.%${q}%,number.ilike.%${q}%,uhid.ilike.%${q}%`,
            )
            .limit(10)

          if (error) throw error

          const mappedPatients: PatientInfo[] = (data || []).map((p: PatientDetailFromSupabase) => ({ // Explicit cast
            uhid: p.uhid,
            name: p.name || "Unknown",
            phone: p.number ? String(p.number) : "N/A",
            age: p.age || null,
      ...
      try {
        ...
        const { data: opdData, error: opdError } = await supabase
          ...
          .select(
            ...
            bill_no,
            uhid,
            patient_detail!inner(uhid, name, number, age, gender, address)
          `,
          )
        ...
        // Explicitly cast to OPDRegistrationSupabase[] for type safety
        const mappedOpd: OPDAppointmentDisplay[] = (opdData as OPDRegistrationSupabase[] || []).map((appt) => {
          ...
          const opdDateIST = toIST(parseISO(appt.date)); // Convert the 'date' field too from UTC

          return {
            ...appt, // Spread the original data
            type: "OPD",
            id: String(appt.opd_id), // Ensure id is string
            patientId: (appt.patient_detail as PatientDetailFromSupabase)?.uhid || "N/A", // Cast patient_detail
            name: (appt.patient_detail as PatientDetailFromSupabase)?.name || "Unknown",
            phone: (appt.patient_detail as PatientDetailFromSupabase)?.number ? String((appt.patient_detail as PatientDetailFromSupabase).number) : "N/A",
            date: format(opdDateIST, "yyyy-MM-dd"), // Use the converted 'date' field
            ...
            payment: (appt.payment_info as IPayment) || {
              cashAmount: 0, createdAt: "", discount: 0, onlineAmount: 0, paymentMethod: "cash", totalCharges: 0, totalPaid: 0,
            },
            message: appt["additional Notes"] || "",
            patient_uhid_from_opd_table: appt.uhid,
            additional_notes: appt["additional Notes"], // Ensure this is mapped correctly
            bill_no: appt.bill_no, // Use as number as per DB schema
            appointment_type: (appt as any).appointment_type, // Cast to any to access if it exists
            visit_type: (appt as any).visit_type, // Cast to any to access if it exists
          };
        })
        setOpdAppointments(mappedOpd)

        // Fetch IPD data with join
        ...
        const mappedIpd: IPDAppointmentDisplay[] = (ipdData as IPDRegistrationSupabase[] || []).map((ipdRecord) => {
          ...
          return {
            ...ipdRecord, // Spread original IPD data
            type: "IPD",
            id: String(ipdRecord.ipd_id),
            patientId: (ipdRecord.patient_detail as PatientDetailFromSupabase)?.uhid || "N/A",
            name: (ipdRecord.patient_detail as PatientDetailFromSupabase)?.name || "Unknown",
            phone: (ipdRecord.patient_detail as PatientDetailFromSupabase)?.number ? String((ipdRecord.patient_detail as PatientDetailFromSupabase).number) : "N/A",
            totalAmount: totalServiceAmount,
            totalDeposit: totalDeposit,
            totalRefunds: totalRefunds,
            discount: totalDiscount,
            remainingAmount: remaining,
            roomType: (ipdRecord.bed_management as { room_type: string })?.room_type || "N/A", // Explicitly cast bed_management
        ...
        // Fetch OT data with join
        const { data: otData, error: otError } = await supabase
          .from("ot_details")
          .select(
            `
            id, ipd_id, uhid, ot_type, ot_notes, ot_date, created_at,
            patient_detail!inner(uhid, name, number)
          `,
          )
          .gte("created_at", startUtc)
          .lte("created_at", endUtc)
        ...
        const mappedOt: OTAppointmentDisplay[] = (otData as OTDetailsSupabase[] || []).map((otRecord) => {
          // ot_date and created_at in ot_details are timezone-aware (Asia/Kolkata default), so convert them for display
          const otDateIST = toIST(parseISO(otRecord.ot_date));
          const createdAtIST = toIST(parseISO(otRecord.created_at));

          return {
            ...otRecord, // Spread original OT data
            type: "OT",
            id: otRecord.id,
            patientId: (otRecord.patient_detail as PatientDetailFromSupabase)?.uhid || "N/A",
            name: (otRecord.patient_detail as PatientDetailFromSupabase)?.name || "Unknown",
            phone: (otRecord.patient_detail as PatientDetailFromSupabase)?.number ? String((otRecord.patient_detail as PatientDetailFromSupabase).number) : "N/A",
    ...
    fetchAppointments()
  }, [filters.searchQuery, currentDateRange]) // Re-run effect when currentDateRange changes
    ...
    async (uhid: string) => {
      ...
      try {
        const { data: opdData, error: opdError } = await supabase
          ...
          .select(
            `
            opd_id, created_at, date, refer_by, "additional Notes", service_info, payment_info, bill_no, uhid,
            patient_detail!inner(uhid, name, number)
          `, // Removed age, gender, address from here to match partial patient_detail in OT.
            // If full patient details are needed consistently, fetch from patient_detail table.
          )
          .eq("uhid", uhid)
        ...
        const mappedOpd: OPDAppointmentDisplay[] = (opdData as OPDRegistrationSupabase[] || []).map((appt) => {
          ...
          return {
            ...
            patientId: (appt.patient_detail as PatientDetailFromSupabase)?.uhid || "N/A",
            name: (appt.patient_detail as PatientDetailFromSupabase)?.name || "Unknown",
            phone: (appt.patient_detail as PatientDetailFromSupabase)?.number ? String((appt.patient_detail as PatientDetailFromSupabase).number) : "N/A",
        ...
        const { data: ipdData, error: ipdError } = await supabase
          ...
          .select(
            ...
            bed_management(room_type),
            patient_detail!inner(uhid, name, number)
          `, // Removed age, gender, address from patient_detail here too
          )
          .eq("uhid", uhid)
          .order("created_at", { ascending: false })
        ...
        const mappedIpd: IPDAppointmentDisplay[] = (ipdData as IPDRegistrationSupabase[] || []).map((ipdRecord) => {
          ...
          return {
            ...
            discount: totalDiscount,
            remainingAmount: remaining,
            roomType: (ipdRecord.bed_management as { room_type: string })?.room_type || "N/A",
            admission_date: ipdRecord.admission_date,
            admission_time: ipdRecord.admission_time,
          }
        })
  ...
  const getFilterTitle = () => {
    ...
    switch (filterType) {
      case "today":
        // Display today's date in IST
        return `Today's Data (${format(toIST(new Date()), "MMM dd, yyyy")})`;
      case "month":
  ...
  return (
    <Layout>
      <main className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
              ...
              <>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                  ...
                  <Card className="bg-white shadow-lg rounded-xl p-6 border border-gray-100">
                    ...
                    <User // Corrected from <User> to <Bar>
                      data={chartData}
                      options={{
                        responsive: true,
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="space-y-3">
                            ...
                            <div>
                              ...
                              <p className="font-medium capitalize">
                                {(selectedAppointment as OPDAppointmentDisplay).visit_type}
                              </p>
                            </div>
                          </div>
                          <div className="space-y-3">
        ...
        <Dialog open={patientAppointmentsModalOpen} onOpenChange={setPatientAppointmentsModalOpen}>
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto p-6">
            <DialogHeader>
              <DialogTitle className="2xl font-bold mb-6 flex items-center">
                <User className="p-3 rounded-full mr-4 bg-gradient-to-r from-sky-100 to-blue-100 text-sky-600 h-6 w-6 shadow-md" />
                Appointments for {selectedPatientForAppointments?.name} (UHID: {selectedPatientForAppointments?.uhid})
              </DialogTitle>

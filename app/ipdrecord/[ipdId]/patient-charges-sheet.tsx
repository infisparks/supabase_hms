// Filename: patient-charges-sheet.tsx
"use client";
...
const createInitialRows = (count: number = 15): ChargeRow[] => {
        ...
        <div>
          {rows.map((row, index) => (
            <div key={index} className={`grid ${gridColumnsClass} text-xs text-center border-t border-gray-400`}>
              ...
              <input type="text" value={row.userIdSign} onChange={(e) => handleInputChange(e, index, 'userIdSign')} className="p-2 focus:outline-none focus:ring-1 focus:ring-blue-500 w-full" />
            </div>
          ))}
        </div>
      </div>
